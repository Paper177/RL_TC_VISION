...
# -*- coding: utf-8 -*-
"""
TD3-Live Training Script with Vision (Auto-Restart Version)
集成视觉特征提取，每500个episode自动保存并退出
"""
import numpy as np
import torch
import os
...
def train_td3_Live(
    ...
    log_path = os.path.join(log_dir, f"TD3_Carsim_Vision_{current_time}")
    os.makedirs(log_path, exist_ok=True)
    writer = SummaryWriter(log_dir=log_path)

    try:
        current_dir = os.path.dirname(os.path.abspath(__file__))
        train_file = os.path.join(current_dir, "train_td3_Live_vision.py")
        env_file = os.path.join(current_dir, "env_live_vision.py")
        
        shutil.copy2(train_file, os.path.join(log_path, "train_td3_Live_vision.py"))
        shutil.copy2(env_file, os.path.join(log_path, "env_live_vision.py"))
    ...
    print(f"\n========== Start TD3 Training with Vision (Auto-Restart every {checkpoint_interval} episodes) ==========")
    
    checkpoint_save_dir = os.path.join(script_dir, "checkpoints")
    os.makedirs(checkpoint_save_dir, exist_ok=True)
    
    best_model_save_dir = os.path.join(script_dir, "best_model_save")
        ...
        for episode in range(start_episode, max_episodes):
            ...
            if episode_reward > best_episode_reward*0.9 and episode_reward >=0:
                ...
                if episode_reward > best_episode_reward:
                    ...
                    agent.save_model(os.path.join(best_model_save_dir, f"TD3_Carsim_Vision_{current_time}_{episode}_{best_episode_reward}.pt"))
                    #log_episode_visuals(writer, episode, episode_visual_history, save_dir=log_path, reward=episode_reward)
                    save_episode_data(episode, episode_visual_history, episode_reward, final_info, log_path)
            ...
            if episodes_completed >= checkpoint_interval:
                ...
                print(f"[CHECKPOINT]   python train_td3_Live_vision.py --model \"{checkpoint_path}\"")
                print(f"{'='*60}\n")
                should_exit = True
                break
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='TD3 Training with Vision and Auto-Restart')
    parser.add_argument('--model', type=str, default=None, help='Path to pretrained model')
    parser.add_argument('--checkpoint_interval', type=int, default=480, help='Episodes before auto-save and exit')
