...
if not sbHandle:
    print(f"Error: Failed to connect to shared buffer '{bufferName}'. Make sure CarSim is running and the buffer name matches.")
    # 为了防止后续代码崩溃，我们可以设置 width 和 height 为 0，或者抛出异常
    # 这里我们选择让后续代码在检查 handle 时处理，或者直接停止执行后续的全局代码
    width = 0
    height = 0
else:
